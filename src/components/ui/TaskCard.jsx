...
const TaskCard = ({ task, startDrag, draggedId, setBtns, btns }) => {
    ...
    const optionBtnHandler = e => {
        ...
        setTimeout(() => { setBtns((prev) => ({ ...prev, isTaskOpBtnClicked: true })) }, 0)
    }
                ...
                <div className='relative' >
                    ...
                    {isTaskOpBtnClicked &&
                        <div className="absolute shadow-md shadow-slate-400 bg-white w-[140px] block px-5 py-3 rounded-md text-xs flex flex-col gap-y-3 top-[30px] -right-[70px]">
                            {task.status !== 'done' &&
                                <>
                                    <button className='flex gap-x-3 ' onClick={() => dispatch({ type: 'TASK_DONE', payload: task.id })}>
                            ...
                            <button className='cursor-pointer flex gap-x-3 ' onClick={() => editHandler(task.id)}>
                            ...
                            <button className='flex gap-x-3 text-red-500 cursor-pointer' onClick={(e) => deleteHandler(e, task.id)}>
